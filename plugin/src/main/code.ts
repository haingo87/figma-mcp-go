  ...
  try {
    switch (request.type) {
      ...
      case "get_nodes_info": {
        ...
        const results = nodes
          .filter((n): n is SceneNode => n !== null && n.type !== "DOCUMENT")
      ...
      case "get_local_components": {
        ...
        const allComponents: { id: string; name: string; key: string | null }[] = [];
      ...
      case "get_annotations": {
        ...
        const nodeAnnotations = (n: BaseNode): unknown[] | null => {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const anns = (n as any).annotations;
          return Array.isArray(anns) ? anns : null;
        };
        if (nodeId) {
          const node = await figma.getNodeByIdAsync(nodeId);
          if (!node) throw new Error(`Node not found: ${nodeId}`);
          const mergedAnnotations: { nodeId: string; annotation: unknown }[] = [];
          const collect = async (n: BaseNode) => {
          ...
          return {
            type: request.type,
            requestId: request.requestId,
            data: { nodeId: node.id, name: node.name, annotations: mergedAnnotations },
          };
        } else {
          const annotated: { nodeId: string; name: string; annotations: unknown[] }[] = [];
          const processNode = async (n: BaseNode) => {
            const anns = nodeAnnotations(n);
      ...
      case "scan_text_nodes": {
        ...
        const findText = async (n: BaseNode) => {
          if (n.type === "TEXT") {
            textNodes.push({
              id: n.id,
              name: n.name,
              characters: (n as TextNode).characters,
              fontSize: (n as TextNode).fontSize,
              fontName: (n as TextNode).fontName,
            });
          }
          if ("children" in n) {
            for (const child of (n as ChildrenMixin).children) {
      ...
      case "scan_nodes_by_types": {
        ...
        const root = await figma.getNodeByIdAsync(nodeId);
        if (!root) throw new Error(`Node not found: ${nodeId}`);
        const matchingNodes: unknown[] = [];
        const findByTypes = async (n: BaseNode) => {
          if ("visible" in n && !(n as SceneNode).visible) return;
          if (types.includes(n.type)) {
            matchingNodes.push({
              ...
              type: n.type,
              bbox: {
                x: "x" in n ? (n as SceneNode).x : 0,
                y: "y" in n ? (n as SceneNode).y : 0,
                width: "width" in n ? (n as SceneNode).width : 0,
                height: "height" in n ? (n as SceneNode).height : 0,
              },
            });
          }
          if ("children" in n) {
        ...
        return {
          type: request.type,
          requestId: request.requestId,
          data: { count: matchingNodes.length, matchingNodes, searchedTypes: types },
        };
      }
      default:
